...
#include "messages_lang.h"
#include "mysql_data.h"
#include "levels.h"
#include "gawnuts_default.h"

/*** the ssl libs ( for future versions ) ***/
...
#define DESC_LEN 40 /* length of the description string */
#define TNAME_LEN 16 /* length of the name len */
#define RECAP_NAME_LEN TNAME_LEN*4 /* length of the recap string */
...
#define EMAIL_LEN 30 /* number of characters in email */ 
#define WWW_LEN 50 /* number of characters in www len */
#define TOTAL_TIME_LEN 40 /* length of time len */
#define MAX_DB_TEXT 300 /* length of the text in the database */
...
".semote",".pemote",".set",".newusers",".version",".entpro",".examine",".passwd",
".dmail",".rmail",".smail",".wake",".promote",".cls", ".addbug", ".delbug", 
".vbug", ".suggest", ".dsuggest", ".vsug", ".talkers", ".rules", ".sayto",
 ".status", ".desc", ".cbuff", ".addnews", ".sysinfo", ".dnews", ".duser", 
".url","*"
 };
...
/* user structure */
struct {
	char buff[ARR_SIZE],name[TNAME_LEN],desc[DESC_LEN], recap[RECAP_NAME_LEN]; 
	char login_name[TNAME_LEN],login_pass[TNAME_LEN];
	char site[80],page_file[80];
	char *pro_start,*pro_end;
        int area,listen,level,file_posn,pro_enter,tmp_user_time;
	int buffpos,sock,time,vis,invite,last_input, have_db;
	int idle_mention,logging_in,attleft,prompt,age,tf,echo,colors;
        char email[EMAIL_LEN], www[WWW_LEN], gender[10], loginmsg[DESC_LEN],bday[DESC_LEN];
        char outmsg[DESC_LEN], icq[DESC_LEN];
	} ustr[MAX_USERS];

/* area structure */
struct {
	char name[TNAME_LEN],topic[TOPIC_LEN+1],move[MAX_AREAS];
...
while(1) {
	...
	for (user=0;user<MAX_USERS;++user) {
		...
		if (com_num==-1 && inpstr[0]=='.') {
		        sprintf(mess,"=> [%s:WARNING]: Comando desconhecido!",TALKERNAME);
...
char user[TNAME_LEN];
{
   ...
   FILE *archive;
   char name[TNAME_LEN], filename[30];
...
{
   FILE *archive;
   char filename[30], name[TNAME_LEN];
...
char name2[TNAME_LEN],passwd2[TNAME_LEN];
int f=0;
FILE *fp;
...
{
int i,u;
char filename[80],timestr[31],sitestr[81],levstr[4],type[10];
char tmp_user_time[TOTAL_TIME_LEN],tf[4],char_have_db[10],char_prompt[4],colors[4],echo[4];  
char tmp_lastline[DESC_LEN*4],tmp_bday[DESC_LEN*4];
FILE *fp;

timestr[0]=0;
   
strcpy(char_prompt,"1");
strcpy(echo,"1");
strcpy(colors,"0");
...
strcpy(ustr[user].recap,ustr[user].login_name);
strcpy(ustr[user].email,"NULL");
strcpy(ustr[user].www,"NULL");
strcpy(ustr[user].bday,"0/0/0");
strcpy(ustr[user].gender,"NULL");
strcpy(ustr[user].loginmsg,"NULL");
strcpy(ustr[user].outmsg,"NULL");
strcpy(ustr[user].icq,"NULL");
ustr[user].age=ustr[user].colors=ustr[user].echo=0;
ustr[user].area=0;
ustr[user].listen=1;
ustr[user].vis=1;
...
if (!(fp=fopen(filename,"r"))) {
	...
	strcpy(ustr[user].desc,"- um novato a tremer nos polos...");
	}
else {
	/* load data */
	fgets(timestr,30,fp);
	...
	fgets(levstr,4,fp);
	fgets(ustr[user].email,EMAIL_LEN-1,fp);
        fgets(ustr[user].www,WWW_LEN-1,fp);
        fgets(ustr[user].bday,39,fp);
        fgets(ustr[user].gender,9,fp);
        fgets(ustr[user].loginmsg,DESC_LEN-1,fp);
        fgets(ustr[user].outmsg,DESC_LEN-1,fp);
        fgets(ustr[user].icq,DESC_LEN-1,fp);
        fgets(ustr[user].recap,(DESC_LEN*4)-1,fp);
        fgets(char_have_db,9,fp);
        fgets(char_prompt,3,fp);
        fgets(echo,3,fp);
        fgets(colors,3,fp);
        fgets(tf,3,fp);
        fgets(tmp_user_time,37,fp);
        fclose(fp);
        
        ustr[user].have_db=atol(char_have_db);
        ustr[user].prompt=atol(char_prompt);
        ustr[user].echo=atol(echo);
        ustr[user].colors=atol(colors);
	ustr[user].level=atoi(levstr);
	ustr[user].tf=atoi(tf);
        ustr[user].tmp_user_time=atoi(tmp_user_time);
   
        /* remove newlines */
	timestr[strlen(timestr)-1]=0;
        ustr[user].bday[strlen(ustr[user].bday)-1]=0;
        ustr[user].desc[strlen(ustr[user].desc)-1]=0;
        ustr[user].email[strlen(ustr[user].email)-1]=0;
        ustr[user].www[strlen(ustr[user].www)-1]=0;
        ustr[user].gender[strlen(ustr[user].gender)-1]=0;
        ustr[user].loginmsg[strlen(ustr[user].loginmsg)-1]=0;
        ustr[user].outmsg[strlen(ustr[user].outmsg)-1]=0;
        ustr[user].icq[strlen(ustr[user].icq)-1]=0;
        ustr[user].recap[strlen(ustr[user].recap)-1]=0;
	}
...
switch(ustr[user].level) {
	case NEWBIE: strcpy(type,"EMBRIAO ");  break;
	case USER  : strcpy(type,"OVO ");  break;
	case WIZARD: strcpy(type,"PINGUIM ");  break;
...
if (ustr[user].name[0]!='?') {
        if(strstr(ustr[user].loginmsg,"NULL"))
	   {
	   ...
	   {
	      sprintf(mess,"=> [%s %s ]: %s\n\r",ustr[user].name,ustr[user].desc,ustr[user].loginmsg);
	      write_alluser(user,mess,1,0);
	   }
...
if(test_bday(user))
     {
	sprintf(mess,"\n\r=> [%s:GREATINGS]: Parabens pelo seu aniversario:-) ",TALKERNAME);
	write_user(user,mess);
	sprintf(mess,"=> [%s:ANNOUNCE]: Hoje eh aniversario do usuario %s. Deem os parabens ele !!!\n\r",TALKERNAME,ustr[user].name);
	write_alluser(user,mess,1,0);
     }
...
char *name;
{
char line[80],name2[TNAME_LEN],tmp_user[ARR_SIZE];
FILE *fp;
DIR *dir;
struct dirent *dp;
   
sprintf(tmp_user,"%s.D",name);

   
if((dir=opendir(USERDATADIR))==NULL)
     {
	sprintf(mess,"Erro ao abrir o diretorio %s para leitura da funcao user_exists()\n",USERDATADIR);
	write_syslog(mess,1);
     }
else
     {   
...
{
time_t tm_num;
char timestr[30],filename[80],cons_sql[300];
FILE *fp;
MYSQL my_connection;
int res,tm,min;

time((long*)&tm);
min=((tm-ustr[user].time)/60)+ustr[user].tmp_user_time;   
...
if(!ustr[user].have_db)
     sprintf(cons_sql,"INSERT INTO gawnuts_users(user_date,ip,description,level,email,site_web,bday,gender,icq,name,user_time) VALUES('%s','%s','%s','%d','%s','%s','%s','%s','%s','%s','%d')",timestr,ustr[user].site,ustr[user].desc,ustr[user].level,ustr[user].email,ustr[user].www,ustr[user].bday,ustr[user].gender,ustr[user].icq,ustr[user].name,min);
else          
     sprintf(cons_sql,"UPDATE gawnuts_users SET user_date='%s',ip='%s',description='%s',level='%d',email='%s',site_web='%s',bday='%s',gender='%s',icq='%s',name='%s',user_time='%d' WHERE name='%s'",timestr,ustr[user].site,ustr[user].desc,ustr[user].level,ustr[user].email,ustr[user].www,ustr[user].bday,ustr[user].gender,ustr[user].icq,ustr[user].name,min,ustr[user].name);
   
mysql_init(&my_connection);
if(mysql_real_connect(&my_connection,MYSQL_HOST,MYSQL_USER,MYSQL_PASS,MYSQL_DB,0,NULL,0))
     {
	write_syslog("DB CONNECTION SUCCESS\n",1);
        res = mysql_query(&my_connection,cons_sql);
	if(!res)
	  {
	     sprintf(mess,"Insert %lu rows in the save_stats_query\n",(unsigned long)mysql_affected_rows(&my_connection));
	     ustr[user].have_db=1;
	     write_syslog(mess,1);
	  }
	else
...
fprintf(fp,"%s%s\n%s\n%d\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%d\n%d\n%d\n%d\n%d\n%d\n",timestr,ustr[user].site,ustr[user].desc, ustr[user].level,ustr[user].email,ustr[user].www,ustr[user].bday,ustr[user].gender,ustr[user].loginmsg,ustr[user].outmsg,ustr[user].icq,ustr[user].recap,ustr[user].have_db,ustr[user].prompt,ustr[user].echo,ustr[user].colors,ustr[user].tf,min);
fclose(fp);   
...
char *inpstr;
{
   ...
   char tmp[10],user3[DESC_LEN];
   int u;
   
   inpstr[0]=toupper(inpstr[0]);
   
   if(((u=get_user_num(inpstr))==-1))
     {
	sprintf(mess,"=> [%s:Oops]: Somente o proprio usuario pode ver seu status!!!! ",TALKERNAME);
	write_user(user,mess);
	return;
     }
    ...
    sprintf(mess,"\n\r============================================================\n\r");
    write_user(user,mess);
    sprintf(mess,"=> Profile do usuario %s \n\r",ustr[user].recap);
    write_user(user,mess);
    sprintf(mess,"============================================================\n\r");
    write_user(user,mess);
    sprintf(mess,"=> Descricao : %s\n\r=> Pagina WWW : %s \n\r",ustr[user].desc,ustr[user].www);
    write_user(user,mess);
    sprintf(mess,"=> O seu aniversario e': %s\n\r=> Sua mensagem de login e' : %s\n\r",ustr[user].bday,ustr[user].loginmsg);
    write_user(user,mess);
    if(strstr(ustr[user].gender,"Male"))
      sprintf(tmp,"%s","Masculino");
    else
      sprintf(tmp,"%s","Feminino");
    sprintf(mess,"=> Sua mensagem de saida do talker e' : %s \n\r=> Seu sexo e' %s \n\r",ustr[user].outmsg,tmp);
    write_user(user,mess);
    sprintf(mess,"=> Seu icq e' : %s \n\r=> Seu recap de nick e' : %s\n\r",ustr[user].icq,ustr[user].recap);
    write_user(user,mess);
    sprintf(mess,"=> Sua idade e' : %d anos\n\r",ustr[user].age);
    if(ustr[user].prompt)
      strcpy(tmp,"On");
    else
      strcpy(tmp,"Off");
    sprintf(mess,"=> Prompt %s -",tmp);
    write_user(user,mess);
    if(ustr[user].echo)
      strcpy(tmp,"On");
    else
      strcpy(tmp,"Off");
    sprintf(mess," Echo %s -",tmp);
    write_user(user,mess);
    if(ustr[user].colors)
      strcpy(mess,"On");
    else
      strcpy(mess,"Off");
    sprintf(mess," Colors %s\n\r",tmp);
    write_user(user,mess);
    if(ustr[user].tf)
     {
	sprintf(mess,"=> Voce e' usuario do programa Tinyfuge \n\r");
	write_user(user,mess);
     }
    if(ustr[user].have_db)
     {
	sprintf(mess,"=> Voce ja' possui um registro em nossa base de dados \n\r");
	write_user(user,mess);
     }
    else
     {
	sprintf(mess,"=> Voce ainda nao possui um registro em nossa base de dados \n\r");
	write_user(user,mess);
     }
...
switch(com_num) {
	...
	case 27: 
		if (ustr[user].prompt) {
		        sprintf(mess,"=> [%s:SUCCESS]: Seu prompt agora esta Off",TALKERNAME);
			write_user(user,mess);  
			ustr[user].prompt=0;
		        save_stats(user); break;
			}
                sprintf(mess,"=> [%s:SUCCESS]: Seu prompt agora esta On",TALKERNAME);
		write_user(user,mess);
		ustr[user].prompt=1; save_stats(user); break;
	...
	case 44: cls(user); break;
        ...
        case 54: status(user,inpstr); break;
        case 55: set_desc(user,inpstr); break;
        case 56: cbuff(user); break;
        case 57: add_news(user,inpstr); break; 
        case 58: systeminfo(user); break;
        case 59: delete_news(user,inpstr); break;
        case 60: delete_user(user,inpstr); break;
        case 61: add_url(user,inpstr); break; 
        default: sprintf(mess,"=> [%s]: O que ?",TALKERNAME); write_user(user,mess); break;
	}
}
...
if (ustr[user].name[0]!='?') {
	if(strstr(ustr[user].outmsg,"NULL"))
          { 
	     sprintf(mess,"=> [ SAIU ]: %s %s\n\r",ustr[user].recap,ustr[user].desc);
	     write_alluser(user,mess,1,0);
	  }
         else
          {
	     sprintf(mess,"=> [%s %s]: %s \n\r",ustr[user].recap,ustr[user].desc,ustr[user].outmsg);
	     write_alluser(user,mess,1,0);
	  }
...
char *inpstr;
{
char other_user[ARR_SIZE],name[ARR_SIZE], tmp_user[ARR_SIZE];
int user2,temp;
DIR *dir;
struct dirent *dp;
...
if ((user2=get_user_num(other_user))==-1) 
	...
	if((dir=opendir(USERDATADIR))==NULL)
	  {
	     sprintf(mess,"=> %s nao esta' no Mundo dos Pinguins!\n\r",other_user);
	     write_user(user,mess);
	     sprintf(mess,"=> [%s]: mande um smail para ele! :-)",TALKERNAME);
	     write_user(user,mess);
	     return;
	  }
	else
	  {
	     while(dp=readdir(dir))
		  ...
		  if(!strcmp(dp->d_name,".") || !strcmp(dp->d_name,"..") || !strcmp(dp->d_name,".D")) continue;
	           ...
	           if(strstr(dp->d_name,tmp_user))
		      {
			 sprintf(mess,"=> %s nao esta no talker\n\r",other_user);
		         write_user(user,mess);
			 sprintf(mess,"=> [%s]: mande um smail para ele! :-)",TALKERNAME);
			 write_user(user,mess);
		         (void)closedir(dir);
...
if (lines<1) 
     {
        sprintf(mess,"=> [%s:WARNING]: Uso : .wipe <numero de comentarios a apagar>",TALKERNAME);
	write_user(user,mess);  
	return;
     }
sprintf(filename,"%s/board%d",MESSDIR,ustr[user].area);
   if (!(bfp=fopen(filename,"r"))) 
     {
       sprintf(mess,"=> [%s:WARNING]: %s",TALKERNAME,nocando);	
       write_user(user,mess);  
       return;
     }
...
/* print messages */
sprintf(mess,"=> [%s:SUCCESS]: Voce apagou algumas mensagens do quadro.",TALKERNAME);
write_user(user,mess);
sprintf(mess,"=> [%s]: %s apaga algumas mensagens da board.\n\r",TALKERNAME,ustr[user].name);
if (!ustr[user].vis)
	sprintf(mess,"=> [%s]: Uma mao fantasma apaga uma mensagem do quadro... SPOOKY!\n\r",TALKERNAME);
write_alluser(user,mess,0,0);
}
...
/*** adding some news in the talker ***/
add_news(user,inpstr)
int user;
char *inpstr;
{
FILE *fp, *fp2;
char tmp_day2,tmp_day[3],system_time[15],system_data[15],the_query[200],c,filename2[30],filename[30],name[TNAME_LEN];
MYSQL my_connection;
int res,day;
time_t tm_num;
struct tm *ts;   
...
time(&tm_num);
ts=localtime(&tm_num);   
strftime(system_data,12,"%Y-%m-%d",ts);
strftime(system_time,12,"%H:%M:%S",ts);   
strftime(tmp_day,2,"%w",ts);
sprintf(mess,"%s",tmp_day);
day=atol(tmp_day);

if(strlen(inpstr) > 110) 
     {
	sprintf(mess,"=> [%s:ERROR]: Sua mensagem esta muito grande ... impossivel inseri-la no banco de dados !!!",TALKERNAME);
	write_user(user,mess);
	return;
     }

if (!inpstr[0]) {
	write_user(user,"Uso: .addnews <noticia>"); return;
	}

sprintf(the_query,"INSERT INTO gawnuts_news(news_date,news_user_name,message,news_time,news_day_of_the_week) VALUES('%s','%s','%s','%s','%s')",system_data,ustr[user].name,inpstr,system_time,day_of_the_week[day]);
   
sprintf(filename,"%s",NEWSFILE);
sprintf(filename2,"%s.tmp",NEWSFILE);   

if (!(fp=fopen(filename,"r+"))) {
	sprintf(mess,"%s : news nao pode ser adicionado, IRONWOOD confira o codigo.",syserror);
	write_user(user,mess);
	sprintf(mess,"ERROR: Couldn't open %s message board file to write in write_board()\n",astr[ustr[user].area].name);
	write_syslog(mess,0);
	return;
...
if(mysql_real_connect(&my_connection,MYSQL_HOST,MYSQL_USER,MYSQL_PASS,MYSQL_DB,0,NULL,0))
	...
	if(!res)
	  {
	     sprintf(mess,"Insert %lu rows in the add_news_query\n",(unsigned long)mysql_affected_rows(&my_connection));
	     write_syslog(mess,1);
	  }
	else
	  {
	     ...
	     write_syslog(mess,1);
	     mysql_close(&my_connection);
	     sprintf(mess,"=> [%s:ERROR]: Impossivel executar a query no banco de dados .. nao eh possivel inserir noticias neste momento ... \n\r",TALKERNAME);
	     write_user(user,mess);
	     fclose(fp);
	  }
	...
	sprintf(mess,"[%s:ERROR] : impossivel conectar a base de dados ... nao e' possivel adicionar news neste momento \n",TALKERNAME);
	write_user(user,mess);
	fclose(fp);
	return;
     }

if(!(fp2=fopen(filename2,"w"))) { fclose(fp); return; }   
strcpy(name,ustr[user].name);
if (!ustr[user].vis)  strcpy(name,"Um pinguim");
sprintf(mess,"(%s) De %s: %s\n",timeline(0),name,inpstr);
fputs(mess,fp2);
...
fclose(fp2);
unlink(NEWSFILE);
rename(filename2,NEWSFILE);
   
/* send output */
write_user(user,"=> Voce adicionou uma noticia no talker!\n\r");
sprintf(inpstr,"foi adicionada uma nova noticia, digite .news e divirta-se !!!!  :-)",ustr[user].name);
write_board(user,inpstr); 
}

/*** function to delete news of the talker ***/
delete_news(user,inpstr)
int user;
char *inpstr;
{
   int i,counter,counter1=0;
   char filename[30], line[130];
   FILE *fp, *fp2;
   
   if(!inpstr[0])
     {
	sprintf(mess,"=> [%s:WARNING]: Uso : .dnews <numero de noticias a serem apagadas>",TALKERNAME);
	write_user(user,mess);
	return;
     }
   ...
   if(!(fp=fopen(NEWSFILE,"r")))
	{
	   sprintf(mess,"Nao foi possivel abrir o arquivo de news para a funcao delete_news() le-lo!!!\n");
	   write_syslog(mess,1);
	   sprintf(mess,"=> [%s:ERROR]: Houve um problema ao abrir o arquivo de news para leitura ... ",TALKERNAME);
	   write_user(user,mess);
	   return;
	}
   ...
   if(i>counter) 
     {
       sprintf(mess,"=> [%s:WARNING]: Impossivel deletar este numero de linhas .... ",TALKERNAME);
       write_user(user,mess);
       fclose(fp);
       return;	
     }
   
   sprintf(filename,"%s.tmp",NEWSFILE);
	
   if(!(fp2=fopen(filename,"w+")))
     {
	sprintf(mess,"Nao foi possivel abrir um arquivo temporario para escrita na funcao delete_news()\n");
	write_syslog(mess,1);
	sprintf(mess,"=> [%s:ERROR]: Houve um problema ao abrir um arquivo temporario para escrita na funcao delete_news()",TALKERNAME);
	write_user(user,mess);
	fclose(fp);
	return;
     }
   ...
   while(!feof(fp))
	...
	counter1++;
	fgets(line,125,fp);
	if(counter1<=counter) fputs(line,fp2);
     }
   ...
   unlink(NEWSFILE);
   rename(filename,NEWSFILE);
   
   sprintf(mess,"=> [%s:SUCCESS]: Voce acabou de deletar %d noticias do nosso talker !!! :-)",TALKERNAME,i);
   write_user(user,mess);
   sprintf(mess,"O usuario %s acaba de deletar %d noticias do nosso talker \n",ustr[user].name,i);
   write_syslog(mess,1);
...
sprintf(mess,"\n\r========== Matou sua curiosidade ? ===============\n\r");
write_user(user,mess);
   
}

/*** clear the last five lines of conversation in area ***/
cbuff(user)
int user;
{
   int pos=astr[ustr[user].area].conv_line%NUM_LINES;
   int f;
   
   sprintf(mess,"=> [%s:ANNOUNCE]: Querendo manter segredo sobre o que voce falou nao eh ? :-)",TALKERNAME);
   
   write_user(user,mess);
   
   for(f=0;f<NUM_LINES;++f)
     {
	strcpy(conv[ustr[user].area][pos++],"       ");
	pos=pos%NUM_LINES;
     }
   
   sprintf(mess,"\n\r=> [%s:SUCCESS]: Voce acaba de deletar o buffer desta sala ... \n\r",TALKERNAME);
...
{
if (!inpstr[0]) {
        sprintf(mess,"=> [%s]: A sua descricao e': %s",TALKERNAME,ustr[user].desc);
	write_user(user,mess);  return;
	}
if (strlen(inpstr)>=DESC_LEN) {
	sprintf(mess,"=> [%s:Oops]: Descricao muito grande !",TALKERNAME);
        write_user(user,mess);  return;
	}
strcpy(ustr[user].desc,inpstr);
save_stats(user); 
sprintf(mess,"=> [%s:SUCCESS]: %s, sua descricao foi alterada para < %s >",TALKERNAME,ustr[user].name,ustr[user].desc);   
write_user(user,mess);
}
...
char *inpstr;
{
   if(!inpstr[0])
     {
	sprintf(mess,"=> [%s]: O seu email atualmente cadastrado e': %s",TALKERNAME,ustr[user].email);
	write_user(user,mess);
	return;
     }
    if(strlen(inpstr)>EMAIL_LEN)
     {
	sprintf(mess,"=> [%s:Oops]: Descricao muito grande !",TALKERNAME);
	write_user(user,mess);
	return;
     }
    strcpy(ustr[user].email,inpstr);
    save_stats(user);
    sprintf(mess,"=> [%s:SUCCESS]: %s, seu email agora esta listado como < %s >",TALKERNAME,ustr[user].name,ustr[user].email);
    write_user(user,mess);
 }

/*** set bday of the user ***/
set_bday(user,inpstr)
int user;
char *inpstr;
{
   if(!inpstr[0])
     {
	sprintf(mess,"=> [%s]: Seu aniversario atualmente cadastrado e': %s",TALKERNAME,ustr[user].bday);
	write_user(user,mess);
	return;
     }
   
   sscanf(inpstr,"%s",ustr[user].bday);
   save_stats(user);
   sprintf(mess,"=> [%s:SUCCESS]: Voce mudou sua data de nascimento para %s\n\r",TALKERNAME,ustr[user].bday);
   write_user(user,mess);
   
   if(test_bday(user))
     {
	sprintf(mess,"=> [%s:GREATINGS]: Parabens pelo seu aniversario:-) ",TALKERNAME);
	write_user(user,mess);
	sprintf(mess,"=> [%s:ANNOUNCE]: Hoje eh aniversario do usuario %s. Deem os parabens ele !!!",TALKERNAME,ustr[user].name);
	write_alluser(user,mess,1,0);
     }
...
set_gender(user,inpstr)
int user;
char *inpstr;
{
   if(!inpstr[0])
     {
	sprintf(mess,"=> [%s]: Voce esta cadastrado como < Sexo( genero ) : %s > ",TALKERNAME,ustr[user].gender);
	write_user(user,mess);
	return;
     }
   inpstr[0]=toupper(inpstr[0]);
   
   if(strstr(inpstr,"Male") || strstr(inpstr,"Female"))
      {
	 strcpy(ustr[user].gender,inpstr);
         save_stats(user);
	 if(strstr(inpstr,"Male"))
	    {
	       sprintf(mess,"=> [%s:SUCCESS]: Voce agora esta cadastrado como < Sexo ( genero ) : %s > ",TALKERNAME,"masculino");
               write_user(user,mess);
	    }
	  else
	    {  
	       sprintf(mess,"=> [%s:SUCCESS]: Voce agora esta cadastrado como < Sexo ( genero ) : %s >",TALKERNAME,"feminino");
	       write_user(user,mess);
	    }
      ...
      {
	 sprintf(mess,"=> [%s:Oops]: As opcoes sao male/female/others",TALKERNAME);
	 write_user(user,mess);
	 return;
      }
...
char *inpstr;
{
   if(!inpstr[0])
     {
	sprintf(mess,"=> [%s]: Sua home page e' < %s > ",TALKERNAME,ustr[user].www);
	write_user(user,mess);
	return;
     }
   
   if(strlen(inpstr)>WWW_LEN)
     {
	sprintf(mess,"=> [%s:Oops]: WWW muito grande ... avise ao IronWood !",TALKERNAME);
	write_user(user,mess);
	return;
     }
   
   strcpy(ustr[user].www,inpstr);
   save_stats(user);
   sprintf(mess,"=> [%s:SUCCESS]: Sua pagina www agora neste talker e' %s ",TALKERNAME,ustr[user].www);
   write_user(user,mess);
}


/*** set the message of the login ***/
set_loginmsg(user,inpstr)
int user;
char *inpstr;
{
   if(!inpstr[0])
     {
	sprintf(mess,"=> [%s]: Sua mensagem de login e' < %s > ",TALKERNAME,ustr[user].loginmsg);
	write_user(user,mess);
	return;
     }
   ...
   if(strlen(inpstr)>DESC_LEN)
     {
	sprintf(mess,"=> [%s:Oops]: Sua mensagem de login esta' muito grande !!! ",TALKERNAME);
	write_user(user,mess);
	return;
     }
   
   strcpy(ustr[user].loginmsg,inpstr);
   save_stats(user);
   sprintf(mess,"=> [%s:SUCCESS]: Sua mensagem de login e' < %s > ",TALKERNAME,ustr[user].loginmsg);
   write_user(user,mess);
}

/*** set the out message, the message shows when the user leaves the talker ***/
set_outmsg(user,inpstr)
int user;
char *inpstr;
{
   if(!inpstr[0])
     {
	sprintf(mess,"=> [%s] : Sua mensagem de saida e' < %s > ",TALKERNAME,ustr[user].outmsg);
	write_user(user,mess);
	return;
     }
   ...
   if(strlen(inpstr)>DESC_LEN)
     {
	sprintf(mess,"=> [%s:Oops]: Sua mensagem de saida esta muito grande !!! ",TALKERNAME);
	write_user(user,mess);
	return;
     }
   
   strcpy(ustr[user].outmsg,inpstr);
   save_stats(user);
   sprintf(mess,"=> [%s:SUCCESS]: Sua mensagem de saida do talker e' < %s > ",TALKERNAME,ustr[user].outmsg);
   write_user(user,mess);
...
char *inpstr;
{
   if(!inpstr[0])
     {
	sprintf(mess,"=> [%s]: Seu unin cadastrado e' < %d > ",TALKERNAME,ustr[user].icq);
	write_user(user,mess);
	return;
     }
   strcpy(ustr[user].icq,inpstr);
   save_stats(user);
   sprintf(mess,"=> [%s:SUCCESS]: Seu numero de icq agora e' < %s > ",TALKERNAME,ustr[user].icq);
   write_user(user,mess);
}
...
char *inpstr;
{
   ...
   if(!inpstr[0])
	...
	if(ustr[user].tf)
	  {
	     sprintf(mess,"=> [%s]: Atualmente voce esta cadastrado como usuario do programa tinyfuge.",TALKERNAME);
	     write_user(user,mess);
	     return;
	  }
	else
	  {
	     sprintf(mess,"=> [%s]: Voce esta cadastrado como nao usuario do programa tinyfuge",TALKERNAME);
	     write_user(user,mess);
	     return;
	  }
     }
   ...
   if(strstr(inpstr,"On"))
     {
       sprintf(mess,"=> [%s:SUCCESS]: Agora voce esta cadastrado como usuario do programa tinyfuge",TALKERNAME);
       write_user(user,mess);
       ustr[user].tf=1;
     }
   else
     {
	sprintf(mess,"=> [%s:SUCCESS]: Agora voce esta cadastrado como nao usuario do programa tinyfuge",TALKERNAME);
	write_user(user,mess);
	ustr[user].tf=0;
     }
...
/*** set the prompt option ***/
set_prompt(user,inpstr)
int user;
char *inpstr;
{
   if(!inpstr[0])
	...
	if(ustr[user].prompt)
	  {
	     sprintf(mess,"=> [%s]: Seu prompt esta cadastrado como On ",TALKERNAME);
	     write_user(user,mess);
	     return;
	  }
	else
	  {
	     sprintf(mess,"=> [%s]: Seu prompt esta cadastrado como Off",TALKERNAME);
	     write_user(user,mess);
	     return;
	  }
     }
...
/*** the set echo function ***/
set_echo(user,inpstr)
int user;
char *inpstr;
{
   if(!inpstr[0])
	...
	if(ustr[user].echo)
	  {
	     sprintf(mess,"=> [%s]: Sua funcao de echo esta On ",TALKERNAME);
	     write_user(user,mess);
	     return;
	  }
	else
	  {
	     sprintf(mess,"=> [%s]: Sua funcao de echo esta Off ",TALKERNAME);
	     write_user(user,mess);
	     return;
	  }
     }
   ...
   if(strstr(inpstr,"On"))
	...
	ustr[user].echo=1;
	save_stats(user);
	sprintf(mess,"=> [%s:SUCCESS]: Sua funcao echo agora esta On",TALKERNAME);
	write_user(user,mess);
     }
   else
     {
	ustr[user].echo=0;
	save_stats(user);
	sprintf(mess,"=> [%s:SUCCESS]: Sua funcao de echo agora esta em Off",TALKERNAME);
	write_user(user,mess);
     }
}

/*** the set recap function ***/
set_recap(user,inpstr)
int user;
char *inpstr;
{
   char tmp_recap[DESC_LEN*4], tmp_name[DESC_LEN*4];
   int i;
   
   if(!inpstr[0])
     {
	sprintf(mess,"=> [%s]: Seu atual nome em recap e' < %s > ",TALKERNAME,ustr[user].recap);
	write_user(user,mess);
	return;
     }
   if(strlen(inpstr)>RECAP_NAME_LEN)
     {
	sprintf(mess,"=> [%s:Oops]: Seu nome de recap esta muito grande .... Calma :-) ",TALKERNAME);
	write_user(user,mess);
	return;
     }
   ...
   for(i=0; inpstr[i] != '\0';i++)
       tmp_recap[i]=tolower(inpstr[i]);
   tmp_recap[i]='\0';
   
   tmp_recap[0]=toupper(tmp_recap[0]);
   
   if(!strstr(ustr[user].name,tmp_recap))
     {
	strcpy(tmp_name,ustr[user].name);
	i = strlen(ustr[user].name);
	tmp_name[i-1]=toupper(tmp_name[i-1]);
	sprintf(mess,"=> [%s:Oops]: O seu nome de recap tem que ser a mesma palavra que o nome de login\n\r",TALKERNAME);
	write_user(user,mess);
	sprintf(mess,"=> Exemplo :  %s - %s ",ustr[user].name,tmp_name);
	write_user(user,mess);
	return;
     }
   
   strcpy(ustr[user].recap,inpstr);
   save_stats(user);
   sprintf(mess,"=> [%s:SUCCESS]: Seu nome nome de recap e' < %s >",TALKERNAME,ustr[user].recap);
   write_user(user,mess);
...
FILE *fp;
char *levstr[]={ "EMBRIAO","OVO","PINGUIM","TUX" };
char tmp_asc[DESC_LEN]= { "( Nao configurado )" };
char level_str[5],timestr[30],sitestr[80],desc[80],email[EMAIL_LEN],www[WWW_LEN];
char bday[DESC_LEN],gender[DESC_LEN],loginmsg[DESC_LEN],outmsg[DESC_LEN],icq[DESC_LEN],recap[DESC_LEN*4];
char age[10],char_prompt[4],echo[4],colors[4],tf[4];
int int_level,int_age,int_prompt,int_echo,int_colors,int_tf;
...
if(u==-1)
     {	
       ...
       fgets(sitestr,80,fp);
       fgets(desc,80,fp);
       fgets(level_str,4,fp);
       fgets(email,EMAIL_LEN-1,fp);
       fgets(www,WWW_LEN-1,fp);
       fgets(bday,39,fp);
       fgets(gender,9,fp);
       fgets(loginmsg,DESC_LEN-1,fp);
       fgets(outmsg,DESC_LEN-1,fp);
       fgets(icq,DESC_LEN-1,fp);
       fgets(recap,(DESC_LEN*4)-1,fp);
       fgets(age,9,fp);
       fgets(char_prompt,3,fp);
       fgets(echo,3,fp);
       fgets(colors,3,fp);
       ...
       int_age=atol(age);
       int_prompt=atol(char_prompt);
       int_echo=atol(echo);
       int_colors=atol(colors);
       int_level=atol(level_str);
       int_tf=atol(tf);
	...
	timestr[strlen(timestr)-1]=0; 
        ...
        loginmsg[strlen(loginmsg)-1]=0;
        outmsg[strlen(outmsg)-1]=0;
        icq[strlen(icq)-1]=0;
        recap[strlen(recap)-1]=0;
	bday[strlen(bday)-1]=0;
     }
   else
     {
	strcpy(recap,ustr[u].recap);
	int_level=ustr[u].level;
	strcpy(gender,ustr[u].gender);
	strcpy(bday,ustr[u].bday);
	strcpy(www,ustr[u].www);
	strcpy(email,ustr[u].email);
	strcpy(icq,ustr[u].icq);
     }
   
   
if(strstr(recap,"NULL"))
     strcpy(recap,user2);

sprintf(mess,"\n\r======================================================\n\r");
write_user(user,mess);

sprintf(mess,"=> Profile do pinguim %s \n\r",recap);
write_user(user,mess);   

sprintf(mess,"======================================================\n\r");
write_user(user,mess);   

sprintf(mess,"=> Nivel : %s \n\r",levstr[int_level]);
write_user(user,mess);   
...
if (get_user_num(user2)!=-1) 
     {
	sprintf(mess,"=> %s esta ligado no %s!!\n\r",user2,TALKERNAME);
	write_user(user,mess); 
     }
else
     { 
	if (!(fp=fopen(filename,"r"))) return;
        fgets(line,80,fp);
        sprintf(mess,"=> %s ligou-se pela ultima vez em %s",user2,line);
        write_user(user,mess);
        fclose(fp);
     }

if(strstr(gender,"Male"))
   strcpy(tmp_asc,"masculino");
if(strstr(gender,"Female"))
   strcpy(tmp_asc,"feminino");


sprintf(mess,"=> E'um pinguim do sexo %s\n\r",tmp_asc);
write_user(user,mess);   

sprintf(mess,"=> Este pinguim nasceu no dia %s\n\r",bday);
write_user(user,mess);

sprintf(mess,"=> O email deste pinguim eh %s\n\r",email);
write_user(user,mess);
   
sprintf(mess,"=> Sua pagina www e' : %s \n\r",www);
write_user(user,mess);

sprintf(mess,"=> O unin ( numero ) icq deste usuario e': %s \n\r",icq);
write_user(user,mess);
...
if(u==user)
     {
	sprintf(mess,"=> Opcoes do .set : status < usuario >\n\r");
        write_user(user,mess);
        sprintf(mess,"======================================================\n\r");
        write_user(user,mess);
     }
...
char *inpstr;
{
char filename[80],name[TNAME_LEN],name2[TNAME_LEN];
int user2;
FILE *fp;
...
char *inpstr;
{
char line[81],name[TNAME_LEN],passwd[TNAME_LEN],word[2][ARR_SIZE];
int found;
FILE *fpi,*fpo;
...
char *inpstr;
{
   char command[ARR_SIZE], filename[30], other_user[ARR_SIZE], type[10], tmp_user[ARR_SIZE];
   int user2;
   ...
   if (!inpstr[0]) 
     {
	if(strstr(command,"Gender"))
	   {
	      set_gender(user,inpstr);
	      return;
	   }
	if(strstr(command,"Desc"))
	   {
	      set_desc(user,inpstr);
	      return;
	   }
	if(strstr(command,"Email"))
	   {   
	      set_email(user,inpstr);
	      return;
	   }
	if(strstr(command,"Bday"))
	   {
	      set_bday(user,inpstr);
	      return;
	   }
	if(strstr(command,"Www"))
	   {
	      set_www(user,inpstr);
	      return;
	   }
	if(strstr(command,"Loginmsg"))
	   {
	      set_loginmsg(user,inpstr);
	      return;
	   }
	if(strstr(command,"Outmsg"))
	   {
	      set_outmsg(user,inpstr);
	      return;
	   }
	if(strstr(command,"Icq"))
	   {
	      set_icq(user,inpstr);
	      return;
	   }
	if(strstr(command,"Tf"))
	   {   
	      set_tf(user,inpstr);
	      return;
	   }
	if(strstr(command,"Prompt"))
	   {   
	      set_prompt(user,inpstr);
	      return;
	   }
	...
	if(strstr(command,"Recap"))
	   {   
	      set_recap(user,inpstr);
	      return;
	   }
	   
	sprintf(filename,"%s",SETFILE);
	more(user,ustr[user].sock,filename);
	return;
     }
...
/** shows the information about the linux system ***/
 ...
 */
  ...
  sprintf(mess,"Information about the system running %s\n\r",TALKERNAME);
  write_user(user,mess);
  sprintf(mess,"Uptime : %d dias, %d horas, %d minutos \n\r",days,hours,mins);
  write_user(user,mess);
  sprintf(mess,"Load Avgs: 1min(%ld) 5min(%ld) 15min(%ld)\n", sys_info.loads[0], sys_info.loads[1], sys_info.loads[2]);
  write_user(user,mess);
  sprintf(mess,"Total de Ram: %ldk\tFree: %ldk\n", sys_info.totalram / 1024, sys_info.freeram / 1024);
  write_user(user,mess);  
  sprintf(mess,"Shared Ram: %ldk\n", sys_info.sharedram / 1024);
  write_user(user,mess);
  sprintf(mess,"Buffered Ram: %ldk\n", sys_info.bufferram / 1024);
  write_user(user,mess);  
  sprintf(mess,"Total Swap: %ldk\tFree: %ldk\n", sys_info.totalswap / 1024, sys_info.freeswap / 1024);
  write_user(user,mess);
  sprintf(mess,"Number of processes: %d\n\r", sys_info.procs);
  write_user(user,mess);	
}

/*** to test the bday of the user ***/
test_bday(user)
int user;
{
   char tmp_bday[15], tmp_bday2[15];
   int i;
   time_t tm_num;
   struct tm *ts;
   ...
   time(&tm_num);
   ts=localtime(&tm_num);
   strftime(tmp_bday,14,"%d/%m",ts);
   for(i=0;i<5;i++) tmp_bday2[i]=ustr[user].bday[i];
   tmp_bday2[5]='\0';
   
   if(strstr(tmp_bday,tmp_bday2)) return 1;
   
   return 0;   
}

/*** function to delete users of the talker ***/
delete_user(user,inpstr)
int user;
char *inpstr;
{
   char line[100],filename[30],the_query[50], other_user[ARR_SIZE], name[TNAME_LEN];
   MYSQL my_connection;
   FILE *fp, *fp2;
   int res;
   ...
   if (!inpstr[0]) 
     {
	sprintf(mess,"=> [%s:WARNING]: Uso : .duser <usuario>",TALKERNAME);
	write_user(user,mess);  return; 
     }
   ...
   if(!(user_exists(user,other_user)))
     {
	sprintf(mess,"=> [%s:ERROR]: O usuario %s nao existe em nosso sistema !!! \n\r",TALKERNAME,other_user);
	write_user(user,mess);
	return;
     }
   
   
   sprintf(the_query,"DELETE FROM gawnuts_users WHERE name='%s'",other_user);
   mysql_init(&my_connection);
   ...
   if(mysql_real_connect(&my_connection,MYSQL_HOST,MYSQL_USER,MYSQL_PASS,MYSQL_DB,0,NULL,0))
	...
	if(!res)
	  {
	     sprintf(mess,"Deleted %lu rows in the function delete_user()\n",(unsigned long)mysql_affected_rows(&my_connection));
	     write_syslog(mess,1);
	  }
	else
	  {
	     sprintf(mess,"Delete error %d : %s \n",mysql_errno(&my_connection),mysql_error(&my_connection));
	     write_syslog(mess,1);
	  }
	...
	mysql_close(&my_connection);
     ...
     {
	sprintf(mess,"CONNECTION FAILED \n");
	write_syslog(mess,1);
	if(mysql_errno(&my_connection))
	  {
	     sprintf(mess,"Connection Error %d : %s \n",mysql_errno(&my_connection),mysql_error(&my_connection));
	     write_syslog(mess,1);
	     sprintf(mess,"=> [%s:ERROR]: Impossivel conectar a base de dados. Favor tentar deletar este usuario mais tarde ... \n\r",TALKERNAME);
	     write_user(user,mess);
	     return;
	  }
     }
   sprintf(filename,"%s.tmp",PASSFILE);
   if (!(fp=fopen(PASSFILE,"r"))) {
	sprintf(mess,"%s : Nao consigo abrir o seu arquivo de password...",syserror);
	write_user(user,mess);
	write_syslog("ERROR: Couldn't open password file to read in delete_user()\n",0);
	return 0;
	}
   if(!(fp2=fopen(filename,"w+")))
     {
	sprintf(mess,"%s : nao consigo abrir arquivo temporario para escrita ... \n",syserror);
	write_user(user,mess);
	write_syslog("ERROR : Couldn't open temporary password file to write in delete_user()",0);
	return;
     }
   ...
   while(!feof(fp)) {
	fgets(line,80,fp);
	sscanf(line,"%s ",name);
	if (strcmp(other_user,name)) fputs(line,fp2); 
	} 
   
  fclose(fp);
  fclose(fp2);
  unlink(PASSFILE);
  rename(filename,PASSFILE); 
  sprintf(filename,"%s/%s.D",USERDATADIR,other_user);
  if(fp=fopen(filename,"r"))
     {  
	fclose(fp);
	unlink(filename);
     }
  sprintf(filename,"%s/%s.P",USERDATADIR,other_user);
  if(fp=fopen(filename,"r"))
     {
	fclose(fp);
	unlink(filename);
     }
  sprintf(filename,"%s/%s.M",MAILDIR,other_user);
  if(fp=fopen(filename,"r"))
     { 
	fclose(fp);
	unlink(filename);
   ...
   sprintf(mess,"=> [%s:SUCCESS]: O usuario %s foi deletado de nosso sistema !!! \n\r",TALKERNAME,other_user);
   write_user(user,mess);
   sprintf(mess,"=> [%s:ANNOUNCE]: O usuario %s foi deletado de nosso sistema !!! \n\r",TALKERNAME,other_user);
   write_alluser(user,mess,1,0);
   sprintf(mess,"SUCCESS : %s is deleted by the user %s \n",other_user,ustr[user].name);
   write_syslog(mess,1);
